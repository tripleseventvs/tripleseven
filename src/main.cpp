...
const string strMessageMagic = "Tripleseven Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight < 40) { nSubsidy = 40000000 * COIN; } // TVS IDI 1600000000
    if (pindexBest->nHeight >= 40) { nSubsidy = 0 * COIN; }
    if (pindexBest->nHeight >= 80) { nSubsidy = 100 * COIN; } // 725760000 8640000 moonthly 2880 Blocks daily approx. 7 years approx to secure network 
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
	
    return nSubsidy + nFees;
}
// TVS POW SUPPLY: 50400000+40000000 = 90400000 approx. 55 mining days
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Tripleseven", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "the bch disaster 2018";
        CTransaction txNew;
        txNew.nTime = 1544046281;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x35e9b20496109dabef326ee753e47e4c48bf8f404509ad557f84e86b009c3821"));
        block.print();
